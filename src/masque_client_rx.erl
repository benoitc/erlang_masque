%%% @doc Shared receive-queue rules for queue-mode client sessions.
%%%
...
%% @doc The receive-queue bound from connect opts.
...
%% @doc True when `Buf' holds `Limit' items or more.
-spec is_full(queue:queue(), pos_integer()) -> boolean().
is_full(Buf, Limit) ->
    queue:len(Buf) >= Limit.

%% @doc True when a tunnel ending now leaves queue-mode data unread.
...
%% @doc Mark the end of `Buf': once the owner has read everything
...
%% @doc Enter actions for the `closed' state.
-spec closed_enter() -> [gen_statem:action()].
closed_enter() ->
    [{state_timeout, ?LINGER_MS, linger_expired}].

%% @doc Event handling for the `closed' state. `Buf' is the session's
%% receive queue (ended with {@link close_queue/2}), `OwnerRef' its
