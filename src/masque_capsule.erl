...
%%% `quic_h3_capsule` (RFC 9297 §3.2); this module adds a small
%%% registry identifying which capsule types MASQUE handles natively.
%%%
%%% RFC 9297 §3.3 requires unknown capsule types to be silently
%%% ignored by the receiver, so the default set below is intentionally
%%% empty - RFC 9298 does not define any mandatory capsules. Extension
%%% RFCs (e.g. compressed datagrams, QUIC-aware proxying) can be
...
%% @doc Returns `true` for capsule types MASQUE handles natively, and
%% `false` for extension / unknown types (which must be silently
%% ignored per RFC 9297 §3.3).
