...
check_connect_host(Headers, Path) ->
    case header(<<"host">>, Headers) of
        ...
        HostHeader ->
            case
                {masque_uri:parse_authority_form(Path), masque_uri:parse_authority_form(HostHeader)}
            of
                {{ok, H1, P1}, {ok, H2, P2}} ->
...
reject(Conn, StreamId, Reason, ExtraHeaders) ->
    ...
    _ =
        (try
            h1:send_response(Conn, StreamId, Status, Headers)
        catch
            _:_ -> ok
        end),
    _ =
        (try
            h1:send_data(Conn, StreamId, Body, true)
        catch
            _:_ -> ok
        end),
    _ =
