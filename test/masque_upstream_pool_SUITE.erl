...
-export([h3_pool_shares_one_owner/1,
...
h2_pool_shares_one_owner(Config) ->
    ...
    Sessions = [direct_connect(EgressPort, UdpPort, h2,
                                #{upstream_pool => true})
                || _ <- lists:seq(1, 3)],
...
different_verify_uses_different_owner(Config) ->
    ...
    UdpPort    = ?config(udp_port, Config),
    Sess1 = direct_connect(EgressPort, UdpPort, h3,
                           #{upstream_pool => true,
                             verify => verify_none}),
    Sess2 = direct_connect(EgressPort, UdpPort, h3,
                           #{upstream_pool => true,
                             verify => verify_peer,
...
direct_connect(EgressPort, UdpPort, Transport, Extra) ->
    ProxyURI = iolist_to_binary(["https://localhost:",
...
run_pool_case(Transport, PoolEnabled, TunnelCount, ExpectedOwners, Config) ->
    ...
    UpstreamOpts0 = #{verify => verify_none,
                      transports => [Transport],
                      alpn => [<<"h3">>]},
    UpstreamOpts = case PoolEnabled of
    ...
    try
        Sessions = [open_ingress_tunnel(IngressPort, UdpPort, #{})
...
round_trip(Sess) ->
    Payload = iolist_to_binary(io_lib:format("hello-~p",
