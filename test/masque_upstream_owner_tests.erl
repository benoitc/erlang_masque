...
acquire_issues_request_and_registers_handler_test() ->
    ...
    ?assert(lists:any(fun({request, _}) -> true; (_) -> false end, Calls)),
    ?assert(lists:any(
        fun({set_stream_handler, [SId, Pid]}) ->
...
release_cancels_and_unregisters_test() ->
    ...
    Calls = ?MOCK:calls(Mock),
    ?assert(lists:any(
        fun({unset_stream_handler, [SId]}) -> SId =:= StreamId;
           (_) -> false
        end, Calls)),
    ?assert(lists:any(
        fun({cancel, [SId]}) -> SId =:= StreamId;
           (_) -> false
        end, Calls)),
    ok = ?M:stop(Owner),
    ?MOCK:stop(Mock).
...
session_death_auto_releases_test() ->
    ...
    SessionPid = erlang:spawn(fun() ->
        Self ! {acquired, ?M:acquire_stream(Owner, sample_headers(),
                                             self(), #{})},
        receive die -> exit(boom) end
    end),
    {ok, StreamId, _Conn} = receive {acquired, R} -> R
                            after 2000 -> ct:fail(no_reply) end,
    #{refs := 1} = ?M:info(Owner),
    SessionPid ! die,
...
acquire_over_limit_rejects_test() ->
    ...
    {ok, _Sid, _} =
        ?M:acquire_stream(Owner, sample_headers(), self(), #{}),
    ?assertEqual({error, stream_limit},
                 ?M:acquire_stream(Owner, sample_headers(),
                                    self(), #{})),
    ok = ?M:stop(Owner),
    ?MOCK:stop(Mock).
...
connection_closed_broadcasts_to_all_sessions_test() ->
    ...
    OtherSession = erlang:spawn(fun() ->
        Self ! {sub, self()},
        receive M -> Self ! {other_got, M} end
    end),
    receive {sub, OtherSession} -> ok after 500 -> ok end,
    {ok, _S2, _} =
    ...
    %% session should too.
    receive {h2, _, closed} -> ok
    after 1000 -> ct:fail(no_closed_to_self) end,
    receive {other_got, {h2, _, closed}} -> ok
    after 1000 -> ct:fail(no_closed_to_other) end,
...
conn_death_stops_owner_test() ->
    ...
    receive {h2, _, closed} -> ok
    after 1000 -> ct:fail(no_closed_on_conn_death) end.
...
h3_datagram_routes_to_matching_session_test() ->
    ...
    SessionA = erlang:spawn(
        fun() -> Self ! {a_stream, ?M:acquire_stream(Owner,
            sample_headers(), self(), #{})},
                  receive {quic_h3, _, {datagram, Sid, P}} ->
                      Self ! {a_got, Sid, P}
                  end
        end),
    SessionB = erlang:spawn(
        fun() -> Self ! {b_stream, ?M:acquire_stream(Owner,
            sample_headers(), self(), #{})},
                  receive {quic_h3, _, {datagram, Sid, P}} ->
                      Self ! {b_got, Sid, P}
                  end
        end),
    {ok, SidA, _} = receive {a_stream, R1} -> R1 after 2000 ->
                        ct:fail(no_a) end,
    {ok, SidB, _} = receive {b_stream, R2} -> R2 after 2000 ->
                        ct:fail(no_b) end,
    %% Simulate h3 delivering a datagram to the conn owner for each
    %% stream; the owner must route by stream_id.
    ?MOCK:simulate(Mock, {send_to_owner, Owner,
                           {quic_h3, Mock, {datagram, SidA, <<"apkt">>}}}),
    ?MOCK:simulate(Mock, {send_to_owner, Owner,
                           {quic_h3, Mock, {datagram, SidB, <<"bpkt">>}}}),
    receive {a_got, SidA, <<"apkt">>} -> ok
    after 1000 -> ct:fail(a_missed) end,
    receive {b_got, SidB, <<"bpkt">>} -> ok
    after 1000 -> ct:fail(b_missed) end,
    _ = SessionA, _ = SessionB,
    ok = ?M:stop(Owner),
    ?MOCK:stop(Mock).
...
start_owner(Transport, Extra) ->
    MockKeys = [peer_settings, request_result, set_handler_result,
                 start_stream_id],
...
wait_dead(Pid, Timeout) ->
    ...
    receive {'DOWN', MRef, process, Pid, _} -> true
    after Timeout -> false
    end.
