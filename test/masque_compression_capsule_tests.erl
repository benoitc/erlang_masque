...
assign_v4_roundtrip_test() ->
    R = #compression_assign{context_id = 2, ip_version = 4,
                            address = {192,0,2,1}, port = 4433},
    ?assertEqual({ok, R},
                 masque_compression_capsule:decode_assign(
                   masque_compression_capsule:encode_assign(R))).

assign_v6_roundtrip_test() ->
    R = #compression_assign{context_id = 4, ip_version = 6,
...
assign_uncompressed_roundtrip_test() ->
    R = #compression_assign{context_id = 6, ip_version = 0,
...
assign_encode_rejects_zero_id_test() ->
    R = #compression_assign{context_id = 0, ip_version = 4,
...
assign_decode_rejects_zero_id_test() ->
    ...
    ?assertEqual({error, zero_context_id},
...
assign_decode_bad_ip_version_test() ->
    Bad = <<2:8, 9:8>>,
    ?assertEqual({error, bad_ip_version},
                 masque_compression_capsule:decode_assign(Bad)).
...
encode1_assign_uses_assign_type_test() ->
    R = #compression_assign{context_id = 2, ip_version = 0,
                            address = undefined, port = undefined},
...
assign_large_context_id_test() ->
    Id = 1234,    %% encoded as a 2-byte varint
    R = #compression_assign{context_id = Id, ip_version = 4,
