...
ppa_parse_v4_only_test() ->
    Headers = [{<<"proxy-public-address">>, <<"\"192.0.2.45:54321\"">>}],
    ?assertEqual(
        {ok, [{{192, 0, 2, 45}, 54321}]},
...
ppa_parse_v6_only_test() ->
    Headers = [{<<"proxy-public-address">>, <<"\"[2001:db8::1234]:54321\"">>}],
    ?assertEqual(
        {ok, [{{16#2001, 16#0DB8, 0, 0, 0, 0, 0, 16#1234}, 54321}]},
        masque_uri_udp_bind:parse_proxy_public_address(Headers)
    ).
...
ppa_parse_malformed_bad_ip_test() ->
    Headers = [{<<"proxy-public-address">>, <<"\"not-an-ip:54321\"">>}],
    ?assertEqual(
        {error, malformed},
        masque_uri_udp_bind:parse_proxy_public_address(Headers)
    ).
...
ppa_parse_malformed_bad_port_test() ->
    Headers = [{<<"proxy-public-address">>, <<"\"192.0.2.45:0\"">>}],
    ?assertEqual(
        {error, malformed},
        masque_uri_udp_bind:parse_proxy_public_address(Headers)
    ).
