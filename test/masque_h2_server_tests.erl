...
validate(Headers) ->
    masque_h2_server:validate(<<"CONNECT">>, ?UDP_PATH, Headers,
                              ?UDP_TPL, ?TCP_TPL, ?IP_TPL, false).
...
udp_reads_authority_and_scheme_test() ->
    Headers = [{<<":protocol">>, ?MASQUE_CONNECT_UDP_PROTOCOL},
               {<<":scheme">>, <<"https">>},
...
ip_reads_authority_and_scheme_test() ->
    Headers = [{<<":protocol">>, ?MASQUE_CONNECT_IP_PROTOCOL},
               {<<":scheme">>, <<"https">>},
               {<<":authority">>, <<"proxy.example:443">>}],
    {ok, Req} = masque_h2_server:validate(
                  <<"CONNECT">>, ?IP_PATH, Headers,
                  ?UDP_TPL, ?TCP_TPL, ?IP_TPL, false),
    ?assertEqual(<<"proxy.example:443">>, maps:get(authority, Req)),
    ?assertEqual(<<"https">>, maps:get(scheme, Req)),
...
udp_bind_reads_authority_and_scheme_test() ->
    Headers = [{<<":protocol">>, ?MASQUE_CONNECT_UDP_PROTOCOL},
               ...
               {<<":authority">>, <<"proxy.example:443">>}],
    {ok, Req} = masque_h2_server:validate(
                  <<"CONNECT">>, <<"/.well-known/masque/udp/*/*/">>, Headers,
                  ?UDP_TPL, ?TCP_TPL, ?IP_TPL, true),
    ?assertEqual(<<"proxy.example:443">>, maps:get(authority, Req)),
    ?assertEqual(<<"https">>, maps:get(scheme, Req)),
