...
client_rejects_path_only_test() ->
    ?assertEqual({error, absolute_uri_required},
...
parse_target_v4_prefix_test() ->
    ?assertEqual({ok, {4, {10,0,0,0}, 24}},
                 masque_uri_ip:parse_target(<<"10.0.0.0/24">>)).

parse_target_v6_prefix_test() ->
    ?assertEqual({ok, {6, {16#2001,16#DB8,0,0,0,0,0,0}, 32}},
...
expand_wildcard_path_test() ->
    ...
    ?assertEqual(<<"https://proxy.example:4443/.well-known/masque/ip/*/*/">>,
                 masque_uri_ip:expand(T, #{target => '*', ipproto => '*'})).

expand_v4_literal_path_test() ->
    {ok, T} = masque_uri_ip:parse_client_template(?ABS_PATH_TPL),
    ?assertEqual(<<"https://proxy.example:4443/.well-known/masque/ip/192.0.2.1/17/">>,
                 masque_uri_ip:expand(T,
                   #{target => {192,0,2,1}, ipproto => 17})).

expand_v4_prefix_escapes_slash_test() ->
    {ok, T} = masque_uri_ip:parse_client_template(?ABS_PATH_TPL),
    ?assertEqual(<<"https://proxy.example:4443/.well-known/masque/ip/10.0.0.0%2F24/0/">>,
                 masque_uri_ip:expand(T,
...
expand_query_form_test() ->
    ...
    ?assertEqual(<<"https://proxy.example:4443/masque?target=*&ipproto=*">>,
                 masque_uri_ip:expand(T, #{target => '*', ipproto => '*'})).

match_query_form_test() ->
    {ok, T} = masque_uri_ip:parse_server_template(
                <<"/masque{?target,ipproto}">>),
    ?assertEqual({ok, #{target => {192,0,2,1}, ipproto => 6}},
                 masque_uri_ip:match(T,
